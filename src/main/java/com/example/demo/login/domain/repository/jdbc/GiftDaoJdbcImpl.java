...
public class GiftDaoJdbcImpl implements GiftDao {
	...
	public int count(String keyword) throws DataAccessException {
		...
		+ "WHERE CONCAT(giftName, description, shop, address, guestName) LIKE '%'||?||'%' AND unavailableFlag = 0", keyword);
	...
	public List<Gift> search(String keyword) throws DataAccessException {
			...
			List<Map<String, Object>>  getList = jdbc.queryForList("SELECT * FROM gift INNER JOIN guest ON gift.guestId = guest.guestId" + " WHERE CONCAT(giftName, description, shop, address, guestName) LIKE '%'||?||'%' AND unavailableFlag = 0", keyword);
	...
	public List<Gift> selectMany() throws DataAccessException {
			...
			List<Map<String, Object>>  getList = jdbc.queryForList("SELECT * FROM gift INNER JOIN guest ON gift.guestId = guest.guestId WHERE unavailableFlag = 0 ORDER BY RANDOM() LIMIT 27");
	...
	public Gift selectOne(int giftId) throws DataAccessException {
		...
		Map<String, Object> map = jdbc.queryForMap("SELECT * FROM gift" + " WHERE giftId = ? AND unavailableFlag = 0", giftId);
