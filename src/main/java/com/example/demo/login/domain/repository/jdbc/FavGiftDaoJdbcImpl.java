...
public class FavGiftDaoJdbcImpl implements FavGiftDao {
	...
	public List<FavGift> selectMany(String userId) throws DataAccessException {
			...
			List<Map<String, Object>>  getList = jdbc.queryForList("SELECT * FROM favGift INNER JOIN gift ON favGift.giftId = gift.giftId INNER JOIN guest ON favGift.guestId = guest.guestId WHERE userId = ? AND favGift.unavailableFlag IS NULL", userId);
	...
	public String searchFavId(String userId, int giftId) throws DataAccessException {
		 ...
		 */
		try {
		Map<String, Object> favIdFind = jdbc.queryForMap("SELECT favId FROM favGift WHERE userId = ? AND giftId = ? AND favGift.unavailableFlag IS NULL", userId, giftId);
	...
	public int count(String userId) throws DataAccessException {
		...
		List<Map<String, Object>>  getList = jdbc.queryForList("SELECT giftId FROM favGift WHERE userId = ? AND favGift.unavailableFlag IS NULL", userId);
	...
	public int delete(String userId, int giftId)throws DataAccessException{
		...
		int rowNumber = jdbc.update("INSERT INTO favGift(unavailableFlag) VALUE(1) WHERE userId = ? AND giftId = ? AND favGift.unavailableFlag IS NULL", userId, giftId);
